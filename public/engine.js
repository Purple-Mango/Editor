...
// Use a hacky approach to private class members until Javascript implements it officially.
function SpawnEngine () {
  // Privatized by scope
  let editor = null
  const session = new Login(null)
  ...
  class _Engine {
    async spawnEditor (id = "") {
      ...
      return new Promise((resolve, reject) => {
         ...
         */
        const editorData = {} // Assume this was fetched from backend and parsed as json

        const index = editorData.indexOf[id]
        if (index < 0) {
          return reject({
            code: 1,
